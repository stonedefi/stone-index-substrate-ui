...
function Management (props) {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      message.error(`Transaction Failed: ${error.toString()}`);
    }
  };
    ...
    <>
      ...
      {currentIndexes.map((index, i) => (
        <SubManage accountPair={accountPair} loadWho={i} indexId={index.stoneId.toNumber()} key={index.stoneId.toNumber()}
          indexName={index.name.toHuman()} components={index.components} owner={index.owner.toHuman()} />
      ))}
    </>
  );
}
...
function SubManage (props) {
  ...
  const { indexName, indexId, components, loadWho, accountPair, owner } = props;
  ...
  const onFinish = async (values) => {
    ...
    try {
      setLoad(loadWho);
      await api.tx.stoneIndex.updateIndex(id, name, assets).signAndSend(fromAcct, txResHandler);
      setLoad(-1);
    } catch (error) {
      message.error(`Transaction Failed: ${error.toString()}`);
    }
